...
export default function PublicBookingLanding() {
  ...
  const [detailsCarId, setDetailsCarId] = useState("");
  ...
  const detailsOption = carAvailabilityOptions.find(
    (option) => option.car.id === detailsCarId
  );
  const detailsCar = detailsOption?.car;
  const detailsCarAvailability = detailsOption?.availability;
  ...
  const whatsappMessage = `Hi ${businessName}, I want to book a self-drive vehicle.`;
  ...
  const getVehiclePublicRules = (car: PublicCar): string[] => {
    const customRules = (car.publicRules || "")
      .split(/\n|\|/)
      .map((rule) => rule.trim())
      .filter(Boolean);
    ...
    return [
      "Extra hourly charges may apply for late return.",
      "Petrol/fuel difference can be deducted from the refundable security deposit.",
      "Scratch, damage, cleaning, toll, Fastag, and challan charges can be deducted from the refundable security deposit.",
      "Final booking is confirmed only after owner approval and payment verification.",
    ];
  };
  ...
  const validateCustomerDetails = () => {
    ...
    if (cleanPhone(formData.alternatePhone).length < 10) {
      toast({
        title: "Enter valid alternate contact number",
        variant: "destructive",
      });
      return false;
    }
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      const requestId = await createBookingRequest({
        ...
        emergencyContactName: "Alternate contact",
        emergencyContactPhone: cleanPhone(formData.alternatePhone),
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Tap a vehicle to view details, rules, charges, and then select it. Booked vehicles stay visible as Not Available.
          </p>
        ...
        {visibleCarOptions.map(({ car, availability }) => {
          ...
          return (
            <button
              key={car.id}
              type="button"
              onClick={() => setDetailsCarId(car.id)}
              className={`text-left rounded-2xl border bg-card overflow-hidden transition hover:shadow-md ${
                availability.available ? "" : "opacity-80 bg-muted/30"
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    {car.showRegistrationNumber && car.registrationNumber && (
                ...
                <p className="text-xs font-semibold text-primary mt-2">
                  Tap to view details and charges
                </p>
    ...
    <div className="rounded-2xl border bg-card p-5 shadow-sm space-y-5">
      ...
      {selectedCar && (
        <div className="rounded-xl border bg-muted/30 p-3 text-sm">
          ...
          {selectedCar.showRegistrationNumber && selectedCar.registrationNumber && (
            <p className="text-xs font-mono uppercase text-muted-foreground">
              Reg: {selectedCar.registrationNumber}
            </p>
          )}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <Label>Alternate contact number <span className="text-destructive">*</span></Label>
          <Input
            required
            type="tel"
            value={formData.alternatePhone}
            onChange={(event) =>
              updateFormData("alternatePhone", event.target.value)
            }
            placeholder="Alternate mobile number"
          />
          <p className="text-xs text-muted-foreground">
            Add a second number in case the main number is not reachable.
          </p>
        </div>
  ...
  const renderVehicleDetailsModal = () => {
    ...
    const image = detailsCar.imageUrl || detailsCar.photoUrls[0] || "";
    const available = Boolean(detailsCarAvailability?.available);
    const displayPrice = getVehicleDisplayPrice(detailsCar);
    const displayDeposit = Number(
      businessSettings?.defaultSecurityDeposit ??
        detailsCar.securityDeposit ??
        0
    );

    return (
      <div className="fixed inset-0 z-[80] bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        <div className="w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border bg-background shadow-2xl">
          <div className="h-52 bg-muted flex items-center justify-center overflow-hidden">
            {image ? (
              <img
                src={image}
                alt={detailsCar.carName}
          ...
          <div className="p-5 space-y-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-2xl font-bold">{detailsCar.carName}</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  {detailsCar.vehicleType} · {detailsCar.fuelType} ·{" "}
                  {detailsCar.transmissionType}
                </p>

                {detailsCar.showRegistrationNumber &&
                  detailsCar.registrationNumber && (
                    <p className="text-sm font-mono uppercase text-muted-foreground mt-1">
                      Registration: {detailsCar.registrationNumber}
                    </p>
                  )}
              </div>
            ...
            <p className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs font-medium text-amber-800">
              Images shown are for reference purpose only. Actual vehicle may
              vary.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border bg-blue-50 border-blue-200 p-3">
                ...
                <p className="text-xl font-bold text-blue-700">
                  ₹{displayDeposit.toLocaleString("en-IN")}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
              <div className="rounded-xl border p-3">
                ...
                <p className="font-semibold">{detailsCar.fuelType}</p>
              </div>

              <div className="rounded-xl border p-3">
                <Gauge className="h-4 w-4 text-primary mb-1" />
                <p className="text-xs text-muted-foreground">Transmission</p>
                <p className="font-semibold">{detailsCar.transmissionType}</p>
              </div>

              <div className="rounded-xl border p-3">
                <Info className="h-4 w-4 text-primary mb-1" />
                <p className="text-xs text-muted-foreground">Status</p>
                <p className={available ? "font-semibold text-emerald-600" : "font-semibold text-red-600"}>
                  {available ? "Available" : "Not Available"}
                </p>
              </div>
            </div>

            {detailsCar.publicDescription && (
            ...
            {detailsCar.features.length > 0 && (
              <div>
                <p className="text-sm font-semibold mb-2">Features</p>
                <div className="flex flex-wrap gap-2">
                  {detailsCar.features.map((feature) => (
                    <span
                      key={feature}
            ...
            <div className="rounded-xl border bg-muted/20 p-4">
              <p className="text-sm font-semibold mb-2">
                Charges and deposit rules
              </p>
              <ul className="space-y-2 text-sm text-muted-foreground">
                {getVehiclePublicRules(detailsCar).map((rule) => (
            ...
            {!available && (
              <p className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-700">
                {detailsCarAvailability?.description ||
                  "This vehicle is not available for selected date/time."}
              </p>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <Button
                ...
                disabled={!available}
                onClick={() => {
                  setSelectedCarId(detailsCar.id);
                  setDetailsCarId("");
                }}
              >
                Select this vehicle
